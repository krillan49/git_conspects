...
puts '                           Что лучше использовать в проекте merge или rebase?'

# Если актуализировать свои свою ветку например с мастером, то лучше всегда юзать merge. Rebase вроде позволяет перезаписывать историю коммитов, что как по мне не очень хорошо бывает в последствии. С merge история коммитов более понятна/прозрачна.

# На rebase история будет чище однако есть вероятность перезаписать изменения. При ребейзе надо делать force push, что опасно, можно какие-то изменения затереть. Ещё если есть много конфликтов в ранних коммитах, то при ребейзе их придётся править несколько раз подряд.

# Хорошая идея использовать squash с merge, с помощью сквош можно получить тоже чистую историю и использовать обычный merge

# Отличная инициатива, squash до одного коммита + rebase
