...
# cherry-pick - команда для того, чтобы смержить только некоторые коммиты из одной ветки в другую, позволяет выбрать конкретные коммиты и применить их к другой ветке.

# 1. Сначала нужно перейти на ветку (тут target-branch), в которую надо добавить коммиты:
# $ git checkout target-branch

# 2. Чтобы выбрать коммиты для объединения, нажно найти SHA-хэши необходимых коммитов (например через "git log", можно неполные с --oneline) к той ветке из которой переносим коммиты (тут source-branch). Запомните или скопируйте хэши нужных коммитов.
# $ git log source-branch

# 3. Использовать команду `git cherry-pick`, чтобы применить нужные коммиты(соответствующие хэши коммитов). Если нужно применить несколько коммитов, можно перечислить их через пробел:
# $ git cherry-pick ce6ae7f 226acc4

# 4. Если при выполнении `cherry-pick` возникнут конфликты, Git сообщит об этом. Разрешите конфликты, отредактировав проблемные файлы, затем добавьте их в индекс/стадию ожидания и завершите процесс слияния с помощью команды:
