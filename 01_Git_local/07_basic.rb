...
# Индекс / staging area - является промежуточной областью, где изменения фиксируются перед выполнением команды `git commit`, оответсвенно термин "добавить в индекс / добавить в staging area" относится к процессу подготовки изменений к фиксации (commit).
...
# 2. Откатить проект до определенных коммитов(все его фаилы соответсвующе меняются), не удаляя коммиты выше. Когда мы откатываемся на этот коммит создается точка возврата. В этом режиме лучше просто просмвтривать и ничего не изменять, тк может нарушится структура и последовательность коммитов.
