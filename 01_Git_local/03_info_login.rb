...
# --global - флаг для установки для всей системы(для любых проектов в ней), без него будет установлено на конкретный проект

# .gitconfig теперь содержит эти данные в папке пользователя в Виноус


# > git config user.name              - посмотреть установленное имя пользователя
# > git config user.email             - посмотреть установленный имэил
